        ...
        <div class="card" v-cloak>
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="file" id="images" name="images[]" multiple @change="imagesSelected($event)" accept="image/png, image/jpeg" />
                ...
                <form class="form-sample" ref="form" @submit.prevent="updateProduct($event)">
                    <div class="row">
                        <div class="col-md-6">
                    ...
                    <button class="btn btn-light" @click="getProduct(id)">Отмена</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
    export default {
        ...
        methods: {
            async getProduct(id) {
                await this.$store.dispatch('product/getProduct', id).then((result) => {
                    this.product = result;
                    this.product.category_id = result.category.id;
                }).catch((err) => { });
            },
            ...
            async updateProduct(event) {
                let data = {
                    id: this.product.id,
                    data: this.sendServerData()
                };
                
                await this.$store.dispatch('product/updateProduct', data).then((result) => {
                    console.log(result);
                }).catch((err) => { });
            },
            ...
            imagesSelected: function(event) {
                ...
                let imagesLength = this.product.images.length;
